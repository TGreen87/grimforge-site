...
export default async function SuccessPage({ searchParams }: Props) {
  ...
  // Fetch Checkout Session + line items safely on the server
  const session = await stripe.checkout.sessions.retrieve(sid, {
    expand: ["line_items.data.price.product"],
  });

  const email =
    (session.customer_details && session.customer_details.email) || session.customer_email || "customer";
    ...
    <main className="container mx-auto max-w-2xl p-6 space-y-6">
      ...
      <section className="rounded-lg border border-border/60 p-4">
        ...
        <ul className="space-y-2">
          {items.map((li) => {
            // @ts-ignore - expanded price/product
            const price = li.price;
            const product = price?.product as any;
            const name = product?.name ?? "Item";
            const qty = li.quantity ?? 1;
            const amount = (li.amount_subtotal ?? 0) / 100;
            return (
        ...
        <div className="mt-4 flex items-center justify-between border-t border-border/50 pt-3 text-sm">
          ...
          <span>${((session.amount_total ?? 0) / 100).toFixed(2)} AUD</span>
        </div>
      </section>

      <div className="text-sm text-muted-foreground">
        <p>If you don’t see a confirmation email, check your spam folder.</p>
      </div>
    </main>
  );
}
