...
- **Plan preview**: Suggested actions now render a multi-step plan with a risk label before you confirm. Use it to sanity-check what will happen; adjustments require updating the context form or action parameters.
...
- **Create a release from media**: Drop art + a short brief into the drawer and run “Create & publish product”. The assistant enriches copy, inserts product/variant/inventory rows, and (optionally) pushes the hero campaign live. Every run logs `assistant.product.full_create`, stores attachment metadata in `assistant_uploads`, and issues an undo token (visible in the drawer) that can delete the generated records if needed.
- **Draft & publish articles**: Use “Draft article” for rapid Journal copy. The assistant generates markdown, saves the article (draft or published), and reports the slug. Run “Publish article” later to promote an existing draft—both emit audit entries and provide undo tokens to drop the draft or revert the publish flag.
- **Refresh the hero**: “Update campaign” rewrites the storefront hero (title, subtitle, highlights, CTAs, layout). Attach new media if needed; the action upserts `campaigns` and activates it when `Activate campaign` is true. Undo either removes the freshly created campaign or restores the previous content/state.
- **Session history**: Every conversation now has a persistent session ID; actions/messages emit entries to `assistant_sessions`, `assistant_session_events`, `assistant_uploads`, and `assistant_action_undos`. Review the feed and attachments under `/admin/assistant/logs` for compliance checks.
